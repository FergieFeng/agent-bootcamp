...
from info_retrieval_prompt_intent import REACT_INSTRUCTIONS
...
Special Rule:
If the question requires *business insight analysis* rather than a single metric lookup,
set "intent": "data_insight".

Use "data_insight" when the question includes:
- comparisons (e.g., “Which segment performs best?”, “compare X vs Y”)
- drivers (e.g., “what drives revenue”, “why is X high”)
- ranking (e.g., “highest revenue”, “top segment”, “best group”)
- multi-month or multi-period analysis (“past 3 months”, “last quarter”)
- segment or product performance questions requiring deeper analytics

Examples that must return intent = "data_insight":
- “Which segment generated the highest revenue in the past 3 months?”
- “I'm a general manager, compare product performance.”
- “Which country is driving growth?”
...
- "intent": one of
    ["quick_metric_lookup", "definition", "trend",
     ...
     "deep_research", "data_insight","other"]
...
async def _main(question: str, history: list[ChatMessage]):
    ...
    For every new user question, we:
      - run Trigger Agent
      - route to MJ_SCOTIA / MJ_BI / Main Orchestrator
    ...
    # We'll compute a single final_answer string in all branches,
    # then build history and yield ONCE at the end (outside the span).
    with langfuse_client.start_as_current_span(name="Agents-SDK-Trace") as span:
        ...
        intent = trigger_result.get("intent", "")
        q_lower = question.lower()

        # Decide routing and compute final_answer
        # --------------------------------------------------------
        # 1) Deep analysis → MJ_SCOTIA
        if "deep analysis" in q_lower:
            insight = run_mj_scotia(question)
            final_answer = f"[Deep Analysis]\n\n{insight}"
            span.update(route="MJ_SCOTIA")

        # 2) Normal BI questions → MJ_BI
        elif intent in ["data_insight", "quick_metric_lookup", "segment_behavior"]:
            insight = analyze_business_insight(question)
            final_answer = f"[Business Insight Worker]\n\n{insight}"
            span.update(route="MJ_BI")

        # 3) Everything else → Main Orchestrator
        else:
            orchestrator_input = {
            ...
            orchestrator_answer = result_stream.final_output
            final_answer = f"[Main Orchestrator]\n\n{orchestrator_answer}"
            span.update(route="MainOrchestrator")

        # Log final output to Langfuse
        span.update(output=final_answer)

    # -------- Build stateless history & return (outside span) --------
...
demo = gr.ChatInterface(
    _main,
    title="Scotiabank-Two: Actionable Insights Engine",
    type="messages",
    examples=[
        "I am general manager, how many active clients made at least one transaction this month?",
        "Definition of active client",
        "Definition of Average Revenue per Client"
        "What segments do we have, and what are revenue as of last month-end?",
        "I'm a general manager. Which segment generate highest revenue for past 3 month?",
        "Deep analysis: How do total loans balance and total deposit balance jointly impact revenue by segment?",
    ],
)
