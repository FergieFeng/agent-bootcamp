...
async def _main(question: str, history: list[ChatMessage]):
    ...
    with langfuse_client.start_as_current_span(name="Agents-SDK-Trace") as span:
        ...
        # ==== Direct MJ_BI Worker Integration ====
        if trigger_result.get("intent") == "data_insight":
            insight = analyze_business_insight(question)

            new_history = [
                ChatMessage(role="user", content=question),
                ChatMessage(role="assistant", content=insight),
            ]
...
demo = gr.ChatInterface(
    ...
    examples=[
        ...
        "What segments do we have, and what are their total balances as of last month-end?",
    ],
)
