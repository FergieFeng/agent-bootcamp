...
class AsyncWeaviateKnowledgeBase:
    ...
    async def search_knowledgebase(self, keyword: str) -> SearchResults:
        ...
        hits = []
        for obj in response.objects:
            # 1) Read fields from your Weaviate object
            text_value = obj.properties.get("searchable_embedding_text", "") or ""
            data_role = obj.properties.get("data_role", None)
            business_tags = obj.properties.get("business_tags", None)

            # 2) Build the hit object using those values
            #    - title: short preview of the offer text
            #    - section: store data_role here
            #    - highlight.text: <= 80 characters, with optional tags appended
            snippet = text_value[:80]  # hard cap at 80 characters

            if business_tags:
                # Optional: append tags at the end of the snippet
                snippet = f"{snippet} [tags: {business_tags}]"

            hit = {
                "_source": {
                    "title": text_value[:80],  # short title-like preview
                    "data_role": data_role,  # <-- your data_role here
                    "business_tags":business_tags # <-- business_tag here
