...
- You receive a JSON string with:
    {{
      "original_question": "<raw natural language>",
      "parsed_intent": {{ ... Trigger Agent JSON ... }}
    }}

Use "parsed_intent" to decide what to do:

- If parsed_intent.intent == "definition" and parsed_intent.metric == "columns":
    - Call the tool `get_dataset_columns` to answer questions about what
      columns exist in the dataset.

- If parsed_intent.intent == "quick_metric_lookup"
  and parsed_intent.metric == "active_clients":
    - Call the tool `count_active_clients_last_month` to get the number
      of active clients last month, then explain the definition you used.

- For deeper / external context that is not in the synthetic dataset,
  you may call the `search` tool (Weaviate-based knowledgebase) to
  retrieve additional information.

Always answer only the current user question. Do not repeat the entire
previous conversation. Use short, focused answers.
  
DATASET SUMMARY (from Hugging Face: {BANK_DATASET_ID}):
...
main_agent = agents.Agent(
    name="MainAgent",
    instructions=MAIN_AGENT_INSTRUCTIONS,  # <-- important
    tools=[
